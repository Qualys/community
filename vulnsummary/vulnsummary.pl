...
# This script executes and saves a scan or retrieves a stored
# scan from the Qualys servers and writes a summary of all the
# vulnerabilities found, as a comma-separated list to stdout, suitable
# for importing into a spreadsheet program.

# This script demonstrates how to work with the Base-64 encoding of the
...
sub vuln {
	...
	printf "\"%s\",%s,\"%s\"\r\n", $vuln->parent->parent->parent->att("value"), $vuln->att('severity'), $vuln->first_child('TITLE')->trimmed_text;
}
...
if ($ARGV[2] =~ /^scan/) {
	...
	printf STDERR "Fetching %s report\n", $ARGV[2];
} else {
	# Run scan case
	...
	printf STDERR "%s (this will take a few minutes)\n", $msg;
}
